...
fn decode_clip(
    ...
    let drain = |decoder: &mut Box<dyn decode::Decoder>,
     ...
     -> Result<Flow> {
        ...
        {
            match handle_frame(clip, cfg, presentation.as_ref(), filters, frame, senders, rt, src_idx, total, joined)? {
            ...
            .context("demuxing next video sample in decode pump")?
        {
            Some(sample) => {
                ...
                if end_sample.is_some_and(|end| idx >= end) {
                    return drain(&mut decoder, &mut filters, &mut src_idx, total, joined);
                }
                ...
                {
                    match handle_frame(clip, cfg, presentation.as_ref(), &mut filters, frame, senders, rt, &mut src_idx, total, joined)? {
                        FrameAction::Continue => {}
                        FrameAction::ClipDone => return Ok(Flow::Continue),
                        FrameAction::StopAll => return Ok(Flow::AllReceiversClosed),
                    }
            ...
            None => return drain(&mut decoder, &mut filters, &mut src_idx, total, joined),
        }
    }
}
...
fn handle_frame(
    ...
    if presented >= clip.start_frame {
        let mut normalized = normalize_frame(cfg, filters, frame)?;
