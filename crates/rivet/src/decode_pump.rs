...
fn decode_clip(
    ...
    let drain = |decoder: &mut Box<dyn decode::Decoder>,
     ...
     -> Result<Flow> {
        ...
        {
            match handle_frame(clip, cfg, presentation.as_ref(), filters, colour, sdr_to_hdr.as_ref(), frame, senders, rt, src_idx, total, joined)? {
    ...
    loop {
            ...
            .context("demuxing next video sample in decode pump")?
        {
            Some(sample) => {
                ...
                {
                    match handle_frame(clip, cfg, presentation.as_ref(), &mut filters, &mut colour, sdr_to_hdr.as_ref(), frame, senders, rt, &mut src_idx, total, joined)? {
                        FrameAction::Continue => {}
                        FrameAction::ClipDone => return Ok(Flow::Continue),
                        FrameAction::StopAll => return Ok(Flow::AllReceiversClosed),
                    }
