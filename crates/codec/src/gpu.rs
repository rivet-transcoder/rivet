...
/// Map a 0-based Intel GPU index (as returned by `detect_intel`) to the
/// libvpl `AdapterSelector::DrmRenderNode` value that targets the same
/// physical adapter. O(1) HashMap lookup against a global cache built
...
pub fn adapter_selector_for_gpu_index(
    ...
    let map = adapter_render_node_map()?;
    match map.get(&gpu_index) {
        Some(&render_node) => Ok(shiguredo_vpl::AdapterSelector::DrmRenderNode(render_node)),
        None => anyhow::bail!(
            "QSV adapter selector: gpu_index={gpu_index} not in the GPU→render-node map \
             (map size={}). Is /dev/dri exposed? Is libmfx-gen installed?",
            map.len()
        ),
    }
}

/// Parse a sysfs PCI BDF string. Accepts both the full form
/// `"domain:bus:device.function"` (e.g. `"0000:03:00.0"`) and the
/// abbreviated form `"bus:device.function"` (e.g. `"03:00.0"`); the
...
fn parse_bdf(bdf: &str) -> Option<(u32, u32, u32, u32)> {
    ...
    let (domain, rest) = if colon_count >= 2 {
        bdf.split_once(':')?
    } else {
        ("0", bdf)
    };
...
/// Cached map from our Intel `gpu_index` (as detected by
/// `detect_intel`) to the libvpl DRM render node value. Built once
/// from a single `detect_gpus()` + `list_adapters()` pair, then
/// served O(1) on every subsequent QSV session creation. Without
/// this we'd walk `/sys/bus/pci/devices` and call NVML on every
/// per-segment encoder / decoder construction.
///
/// Correlation strategy:
/// 1. PCI-address match (primary): for each Intel device in our
///    inventory, find the libvpl `AdapterInfo` whose `pci_address`
///    matches and record `index → drm_render_node`.
/// 2. Ordinal fallback (secondary): any Intel device not matched by
...
fn adapter_render_node_map() -> anyhow::Result<&'static std::collections::HashMap<u32, u32>> {
    ...
    #[cfg(target_os = "linux")]
    {
        let cached = MAP.get_or_init(build_render_node_map);
        if cached.is_empty() {
            anyhow::bail!(
                "GPU→render-node map is empty (shiguredo_vpl::list_adapters() returned \
                 no Intel HW implementations, or detect_intel() found no Intel cards)"
            );
        }
        Ok(cached)
    }
    ...
    {
        let _ = &MAP;
        anyhow::bail!("QSV adapter selection is Linux-only")
    }
}
...
fn build_render_node_map() -> std::collections::HashMap<u32, u32> {
    ...
    let adapters = match shiguredo_vpl::list_adapters() {
        ...
        Err(e) => {
            tracing::warn!(
                error = %e,
                "shiguredo_vpl::list_adapters() failed; QSV adapter selection will fail"
            );
            return map;
        }
    };
    ...
    let our_gpus = detect_gpus();
    let intel_gpus: Vec<&GpuDevice> = our_gpus
        .iter()
        .filter(|g| matches!(g.vendor, GpuVendor::Intel))
        .collect();
    let mut unmatched_indices: Vec<u32> = Vec::new();
    for dev in &intel_gpus {
        if let Some(pci) = parse_bdf(&dev.host_pci_address)
            && let Some(hit) = adapters.iter().find(|a| {
                a.pci_address.domain == pci.0
                    && a.pci_address.bus == pci.1
                    && a.pci_address.device == pci.2
                    && a.pci_address.function == pci.3
            })
        {
            map.insert(dev.index, hit.drm_render_node);
            continue;
        }
        unmatched_indices.push(dev.index);
    }
    // Ordinal fallback for anything that didn't correlate by PCI.
    for &idx in &unmatched_indices {
        if let Some(hit) = adapters.get(idx as usize) {
            tracing::warn!(
                gpu_index = idx,
                drm_render_node = hit.drm_render_node,
                "QSV adapter map: PCI-address correlation failed; using ordinal match"
            );
            map.insert(idx, hit.drm_render_node);
        }
    }
    tracing::info!(
        intel_gpus = intel_gpus.len(),
        adapters_enumerated = adapters.len(),
        mapped = map.len(),
        unmatched_pci = unmatched_indices.len(),
        "QSV adapter render-node map built"
    );
    map
}
...
pub fn supports_av1_encode(device: &GpuDevice) -> bool {
    match device.vendor {
        ...
        // `intel_label_from_device_id` — we match family names rather
        // than SKUs so a new B770 / Lunar Lake variant works without
        // a code change.
        GpuVendor::Intel => {
            let name = device.name.to_lowercase();
            name.contains("arc")             // Alchemist / Battlemage / future
                || name.contains("battlemage")
                || name.contains("alchemist")
                || name.contains("meteor lake")
                || name.contains("lunar lake")
                || name.contains("arrow lake")
                || name.contains("core ultra") // covers ML / LL / AL umbrella
        }
    }
}
...
mod parse_bdf_tests {
    ...
    #[test]
    fn full_form_with_domain_parses() {
        assert_eq!(parse_bdf("0000:03:00.0"), Some((0, 3, 0, 0)));
        assert_eq!(parse_bdf("0001:af:1f.7"), Some((1, 0xaf, 0x1f, 7)));
    }

    #[test]
    fn short_form_without_domain_parses_as_domain_zero() {
        // The sysfs reader in this module strips the leading "0000:"
        // when present, so callers pass the abbreviated form. Both
        // shapes have to parse to the same (0, bus, device, function)
        // tuple or PCI-address correlation against libvpl breaks.
        assert_eq!(parse_bdf("03:00.0"), Some((0, 3, 0, 0)));
        assert_eq!(parse_bdf("af:1f.7"), Some((0, 0xaf, 0x1f, 7)));
    }

    #[test]
    fn full_and_short_forms_agree_on_default_domain() {
        assert_eq!(parse_bdf("0000:03:00.0"), parse_bdf("03:00.0"));
    }

    #[test]
    fn malformed_inputs_return_none() {
        assert_eq!(parse_bdf(""), None);
        assert_eq!(parse_bdf("not-a-bdf"), None);
        assert_eq!(parse_bdf("03:00"), None); // missing .function
        assert_eq!(parse_bdf("03.0"), None); // missing :device
        assert_eq!(parse_bdf("gg:00.0"), None); // non-hex
    }
}
